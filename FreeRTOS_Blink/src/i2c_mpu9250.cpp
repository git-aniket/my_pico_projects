...
bool MPU9250::MPU_config(MPU9250_ACCEL_FULL_SCALE acc_scale, MPU9250_GYRO_FULL_SCALE gyro_scale)
{
    ...
    uint8_t config_register_write[5] = {
        ...
        (GYRO_DLP_CFG_250_8),                  // write to register 26 MPU9250_CONFIG
        ...
        (0) | (ACC_DLP_CONFIG_184)             // write to register 29 MPU9250_ACCEL_CONFIG2
    };

    status = i2c_write_timeout_us(this->PORT, MPU9250_ADDRESS_AD0_HIGH, config_register_write, 5, true, 1000);
    if (status != 5)
    {
...
bool MPU9250::get_raw_data()
{
    ...
    status = i2c_write_timeout_us(this->PORT, MPU9250_ADDRESS_AD0_HIGH, &acc_reg, 1, true, 1000);
    if (status != 1)
    {
        if (status == PICO_ERROR_GENERIC)
    ...
    status = i2c_read_timeout_us(this->PORT, MPU9250_ADDRESS_AD0_HIGH, this->raw_data_buffer, MPU_RAW_REG_COUNT, false, MPU_RAW_REG_COUNT * 1000);
    if (status != MPU_RAW_REG_COUNT)
    {
        if (status == PICO_ERROR_GENERIC)
            printf("generic error\n");
        else if (status == PICO_ERROR_TIMEOUT)
